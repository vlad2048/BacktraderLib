...
static class EnumExt
{
	...
	public static IReadOnlyDictionary<K, T[]> GroupInDict<K, T>(this IEnumerable<T> source, Func<T, K> keyFun) where K : notnull => source.GroupBy(keyFun).ToDictionary(e => e.Key, e => e.ToArray());
	//public static IReadOnlyDictionary<K, U[]> GroupInDict<K, T, U>(this IEnumerable<T> source, Func<T, K> keyFun, Func<T, U> valFun) where K : notnull => source.GroupBy(keyFun).ToDictionary(e => e.Key, e => e.SelectA(valFun));
