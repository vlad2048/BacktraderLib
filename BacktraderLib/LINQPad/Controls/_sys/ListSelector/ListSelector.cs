...
static class ListSelector
{
	...
	static IRoVar<Tag> Format<T, U>(IRoVar<T[]> Δsource, Func<T, U> dispFun, IRwVar<T> Δrx)
	{
		var props = GetProps<U>();
		var Δtable = Var.Expr(() => MakeTable(props, Δsource.V.SelectA(e => new Row<T>(e, dispFun(e).GetRowTags(props))), Δrx.V, Δrx));
		return Δtable;
	}
	static Tag MakeTable<T, U>(Prop<U>[] props, Row<T>[] rows, T rxVal, IRwVar<T> Δrx) =>
		new("table")
		{
			Kids =
			[
				...
				new Tag("tbody")
				{
					Kids = rows.SelectA(
						row => new Tag("tr")
						{
							Kids = row.Cells,
							Id = IdGen.Make(),
							OnClick = () => Δrx.V = row.Obj,
							Style = [
								..A("cursor: pointer"),
								..If(rxVal.Equals(row.Obj), "background-color:#333"),
							],
						}
					),
				},
